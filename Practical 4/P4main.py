...
init_hilat = {
    ...
    'DIC': (38700e15/12)/V_ocean, # TODO is this correct units?
    'TA': 3.1e18/3,
    'tau_CO2': 2.
}
...
def ocean_model(lolat, hilat, deep, atmos, tmax, dt):
    ...
    for i in tqdm(range(1, time.size)):
        ...
        for box in [hilat, lolat]:
            ...
            fluxes[f'dDIC_{boxname}'] = (box['V'] / box['tau_CO2']) * (box['CO2'][last] - box['K0'][last]*atmos['pCO2'][last]*1e-3) * dt
        ...
        atmos['moles_CO2'][i] = atmos['moles_CO2'][last] + fluxes[f'dDIC_hilat'] + fluxes[f'dDIC_lolat'] + fluxes['emissions'][last]
